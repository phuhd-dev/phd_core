    ...
    data.keys.toList().asMap().forEach((index, key) {
      ...
      if (value is Map) {
        ...
        if (value.length == 0) {
          ...
          LogUtil.d('${_indent(tabs)} ]${isLast ? '' : ','}', tag: tag);
        }
      } else {
        final msg = value.toString().replaceAll('\n', '');
        LogUtil.d('${_indent(tabs)} $key: $msg${!isLast ? ',' : ''}', tag: tag);
      }
    });

    LogUtil.d('$initialIndent}${isListItem && !isLast ? ',' : ''}', tag: tag);
  }
  ...
  static void _printList(List list, {String tag = tag, int tabs = 1}) {
    list.asMap().forEach((i, e) {
      ...
      if (e is Map) {
        ...
        LogUtil.d('${_indent(tabs + 2)} $e${isLast ? '' : ','}', tag: tag);
      }
    });
  }
